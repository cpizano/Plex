...
public:
  TestPipeHandler()
    : srv_pipe_(plx::ServerPipe::Create(L"fracus_been", plx::ServerPipe::overlapped)),
      transactions_(0) {
  }
...
void Test_Version::Exec() {
  ...
  CheckEQ(v.minor(), 22);
  CheckEQ(v.rev(), 3);
  CheckEQ(v.build(), 1124);
  auto s = v.to_string();
  CheckEQ(s, test_str);

  plx::Version vv;
  CheckEQ(vv.to_string(), "0.0.0.0");

  int r = plx::Version::Compare(v, vv);
  CheckEQ(r, 5);
}
...
class JobInfoTestHandler : public plx::JobObjEventHandler {
public:
...
void Test_JobObject::Exec() {
  ...
  plx::JobObjecNotification notif(&cp, &handler);

  auto job = plx::JobObject::Create(
      L"blueman1", plx::JobObjectLimits(), &notif);
  CheckEQ(job.is_valid(), true);
  CheckEQ(job.status(), 0);

  std::thread t1([&notif] {
    while (true) {
      auto res = notif.wait_for_event(INFINITE);
      if (res == plx::CompletionPort::op_exit)
        break;
    }
  });
